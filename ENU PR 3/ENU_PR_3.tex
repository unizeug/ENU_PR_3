...
\section{2  Amplitudenmodulation}
\begin{quote}
    
    
    \subsection{Labordurchführung}
    \begin{quote}
      Die Amplitudenmodulation mit Träger erhält man durch die Multiplikation
      eines Cosinus (Trägersignal) mit der Summe eines Offsetkonstanten A und
      dem mit einer weiteren Konstanten $\kappa$ gewichteten zu übertragenden
      Signal.
      
      \begin{equation*}
        \begin{split}
        \\
           u_{m}(t)&=A_{c}[u(t)]cos(\omega_{c}t)
        \\
           A_{c}(t)&=A+\kappa u(t) 
        \\
        \end{split}
      ...
      Dies setzen wir im Praktikum folgendermaßen um:
